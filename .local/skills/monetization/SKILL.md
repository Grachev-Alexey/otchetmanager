...
Add `"RevenueCat (best for iOS in-app payments, requires an account)"` as a fourth option **only** if the app is native-mobile / Expo / React Native, or the user asked about IAP on iOS. Never offer RevenueCat for plain web apps.
