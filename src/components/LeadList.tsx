...
import { 
  Search, Edit3, Trash2, Calendar, Phone, Check, 
  Layers, CheckCircle2, XCircle, Info, Filter
} from 'lucide-react';
...
export default function LeadList({ 
  ...
  useEffect(() => {
    fetch('/api/users')
      ...
      .catch(err => console.error('Error in lead list fetching staff:', err));
  }, []);
    ...
    switch (status) {
      ...
      case 'cancelled':
        return { 
          text: 'Отменена', 
          icon: XCircle,
          colorClasses: 'bg-slate-50 border-slate-205 text-slate-500 font-medium'
        };
      default:
        return { 
          text: status, 
